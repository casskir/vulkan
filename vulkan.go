...
func WaitForFences(device Device, fenceCount uint32, pFences []Fence, waitAll Bool32, timeout uint) Result {
	cdevice, _ := *(*C.VkDevice)(unsafe.Pointer(&device)), cgoAllocsUnknown
	cfenceCount, _ := (C.uint32_t)(fenceCount), cgoAllocsUnknown
	cpFences, _ := (*C.VkFence)(unsafe.Pointer((*sliceHeader)(unsafe.Pointer(&pFences)).Data)), cgoAllocsUnknown
...
func AcquireNextImage(device Device, swapchain Swapchain, timeout uint, semaphore Semaphore, fence Fence, pImageIndex *uint32) Result {
	cdevice, _ := *(*C.VkDevice)(unsafe.Pointer(&device)), cgoAllocsUnknown
	cswapchain, _ := *(*C.VkSwapchainKHR)(unsafe.Pointer(&swapchain)), cgoAllocsUnknown
	ctimeout, _ := (C.uint64_t)(timeout), cgoAllocsUnknown
	csemaphore, _ := *(*C.VkSemaphore)(unsafe.Pointer(&semaphore)), cgoAllocsUnknown
...
func DebugReportMessage(instance Instance, flags DebugReportFlags, objectType DebugReportObjectType, object uint, location uint, messageCode int32, pLayerPrefix string, pMessage string) {
	cinstance, _ := *(*C.VkInstance)(unsafe.Pointer(&instance)), cgoAllocsUnknown
	...
	pLayerPrefix = safeString(pLayerPrefix)
	cpLayerPrefix, _ := unpackPCharString(pLayerPrefix)
	pMessage = safeString(pMessage)
	...
	runtime.KeepAlive(pMessage)
	runtime.KeepAlive(pLayerPrefix)
}
